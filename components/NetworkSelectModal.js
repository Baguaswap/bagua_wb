    ...
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center">
      ...
      <div className="relative flex max-h-[80vh] w-full flex-col rounded-t-2xl bg-bg-panel card-border p-5 shadow-glow sm:max-w-sm sm:rounded-2xl">
        <div className="flex items-center justify-between">
          <h3 className="font-display text-base font-bold text-white">Pilih Jaringan</h3>
          <button
            onClick={onClose}
        ...
        <div className="mt-4 flex-1 space-y-2 overflow-y-auto no-scrollbar">
          {filteredNetworks.length === 0 ? (
            <p className="py-8 text-center text-sm text-white/40">Jaringan tidak ditemukan</p>
          ) : (
            filteredNetworks.map((network) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-xl bg-bg-card card-border px-3 py-3 text-left hover:bg-white/5"
                >
                  {network.iconUrl ? (
