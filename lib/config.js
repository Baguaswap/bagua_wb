...
//
// `icon` holds the fallback vector component (from components/icons) shown
// whenever `iconUrl` is null — every network below has one so nothing ever
// falls back to a generic/wrong logo.
export const NETWORKS = [
  ...
  },
  {
    ...
    name: "Tron",
    type: "mainnet",
    // Tron isn't EVM/chainId-based like the networks above — it needs its
    // own (non-MetaMask) wallet connector before it can actually be
    // switched to. Listed here for the chain picker UI; wire up a Tron
    // connector separately when you're ready to support it functionally.
    chainId: null,
    iconUrl: null,
    icon: "TronIcon",
  ...
  },
  {
    ...
    name: "Solana",
    type: "mainnet",
    // Same note as Tron above — Solana is a separate wallet ecosystem
    // (Phantom/Solflare, not MetaMask-style chainId switching).
    chainId: null,
    iconUrl: null,
    icon: "SolanaIcon",
